...
public class CustomStyledPlayerView extends StyledPlayerView {
    ...
    private boolean playingAsAudio = false;
    ...
    // public final ViewGroup videoViewParent;
    ...
    public CustomStyledPlayerView(Context context, AttributeSet attrs, int defStyleAttr) {
        ...
        audioView = findViewById(R.id.exo_audio_layout);
    }

    public void updateScaleDetector() {
        if (Utils.getGestureEnabled() && !playingAsAudio) {
            mScaleDetector = new ScaleGestureDetector(getContext(), getScaleDetector());
        } else
            mScaleDetector = new ScaleGestureDetector(getContext(), getEmptyScaleDetector());

    }

    private ScaleGestureDetector.OnScaleGestureListener getScaleDetector() {
    ...
    private ScaleGestureDetector.OnScaleGestureListener getEmptyScaleDetector() {
        return new ScaleGestureDetector.OnScaleGestureListener() {
            @Override
            public boolean onScaleBegin(ScaleGestureDetector scaleGestureDetector) {
                return false;
    ...
    public void updateGestureDetector() {
        Log.d("valueUsing", "now3");
        if (Utils.getGestureEnabled() && !playingAsAudio) {
            mDetector = new GestureDetectorCompat(getContext(), getTouchEvent());
        } else {
            mDetector = new GestureDetectorCompat(getContext(), getEmptyTouchEvent());
        }
    }
    ...
    public boolean switchAudioMode() {
        if (playingAsAudio) {
    ...
    private GestureDetector.OnGestureListener getTouchEvent() {
                    ...
                    if (gestureOrientation == Orientation.VERTICAL || gestureOrientation == Orientation.UNKNOWN) {
                        ...
                        if (Math.abs(gestureScrollY) > SCROLL_STEP) {
                            ...
                            if (motionEvent.getX() < (float) (getWidth() / 2)) {
                                PlayerActivity.mBrightnessControl.changeBrightness(CustomStyledPlayerView.this, gestureScrollY > 0, canSetAutoBrightness);
                            } else {
                                Utils.adjustVolume(mAudioManager, CustomStyledPlayerView.this, gestureScrollY > 0, canBoostVolume);
                            }
    ...
    private GestureDetector.OnGestureListener getEmptyTouchEvent() {
        return new GestureDetector.OnGestureListener() {
