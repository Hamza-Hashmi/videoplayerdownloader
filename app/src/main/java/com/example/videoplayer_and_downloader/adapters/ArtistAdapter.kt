...
class ArtistAdapter( val callback:(artist:String)->Unit,var nativeAdId: String, var fbNative: String, var keyPriority: Int = 3):RecyclerView.Adapter<RecyclerView.ViewHolder>(),
    NativeAdsCallBack {
    ...
    inner class MyViewHolder(val binding: RowAlbumArtistBinding) : RecyclerView.ViewHolder(binding.root) {
        fun bindImageData(
            singleMedia: MusicModel,
            position: Int
        ) {
            binding.apply {
                this.tvAlbumName.text = singleMedia.artist
            }

            itemView.setOnClickListener {
                singleMedia.artist?.let { it1 -> callback(it1) }
            }
        }
    }
}